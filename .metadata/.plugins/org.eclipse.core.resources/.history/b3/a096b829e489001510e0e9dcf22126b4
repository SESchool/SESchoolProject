...
<%
	...
	String v_isStartEdu		= ( ds.getString("EDUSTART").compareTo(v_todate) > 0)  ? "N": "Y"  ;    
   	...
   	String v_insurayn		= ( v_returnyn.equals("Y" ) && v_documentyn.equals("Y") )? "Y": "N" ;		// 고용보험 적용
...
<!--
	function refundConfirm() {
	  ...
	  <%if(!"01".equals(ds.getString("PAYMETHOD")) ) {  // 카드 결제가 아닌경우 %>
	...
	function validate(f) {		
		...
		<% } else if( "01".equals(ds.getString("PAYMETHOD"))) {  // 카드 결제가 아닌경우 %>				 	
			if(f.p_refundreqdt.value =="" && !f.p_refundstate[0].checked) {
				alert("환불요청일을 입력하세요.");
				return false;
			}
	...
	function changeRefundType(refundtype){
		...
		var v_recogstate = f.p_recogstate.value; 
		if(refundtype == "F" ) {
			if("01" == "<%=payMethod%>" && "<%=v_isStartEdu%>" == "N"  && !(v_recogstate == "C") && !(v_recogstate == "E") && !(v_recogstate == "D") ) {  // 개강전 (카드결제)
...
<BODY leftMargin=0 topMargin=0 marginheight="0" marginwidth="0" >
	...
	<input type="hidden" name="cmd" value="refundWriteForm">
  	...
  	<input type="hidden" name="p_isopensubj" 	value="Y">
			...
			<tr>
				...
				<td colspan="3">
					...
					<% } else { %>
						...
						-
						<input name="p_memberpost2" type="text" size="5" dispName="우편번호" value="<%=v_memberpost2 %>" readonly="readonly"/>
					<% } %>								
	...
	<tr>
		<td class="pad_left_15">
			...
			<%if("01".equals(ds.getString("PAYMETHOD"))) {  // 카드 결제%>
	 		<tr>
				<th>결제구분</th>
				<td>
					<input type="Hidden" name="p_paymethod" value="01"/>
					카드</td>		
				<th>영문이름</th>
				<td></td>
			</tr>	
			...
			<%} else if("02".equals(ds.getString("PAYMETHOD"))) {  // 가상계좌 %>
			<tr>
				<th>결제유형</th>
				<td>
					<select name="p_paymethod">
						<option value="02" selected >가상계좌</option>
						<option value="03">무통장</option>
					</select>
				</td>
				<th>입금은행</th>
			...
			<%} else if("03".equals(ds.getString("PAYMETHOD"))) {%>
			<tr>
				<th>결제유형</th>
				<td>
					<select name="p_paymethod">
						<option value="02">가상계좌</option>
						<option value="03"  selected >무통장</option>
					</select>
				</td>		
				<th>입금은행</th>
				<td><%=ds.getString("BANK_NAME")%></td>
	...
	<tr>
		<td class="pad_left_15">
			...
			<%if("01".equals(ds.getString("PAYMETHOD"))) {%>
			...
			<tr>	
				<th>환불요청일</th>
				<td colspan="3"  >
	             <input name="p_refundreqdt" type="text" size="13" maxlength="10" dispName="환불요청일" value="<%=ds.getString("REFUNDREQDT")%>"/  <%=v_refundtype.equals("F")?"readonly":"" %>> ex)20101030</td>	
				</td>
			</tr>
			<tr>
				<th>환불타입</th>
	...
	<tr>
		<td class="pad_left_15">
			...
			<tr>
				...
				<th>고용보험적용</th>
				<td>
					<input name="p_costsupport"  type="hidden" value="<%=ds.getString("COSTSUPPORT")%>" />
	...
	<tr>
		<td align="center">
			...
			<tr>
			  <td>
				<%if(s_menucontrol.equals("RW") && v_ispayment.equals("1") && !v_refundtype.equals("F") ) {%>
