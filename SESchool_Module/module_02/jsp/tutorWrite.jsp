...
<%
	...
	String v_tutortype 	= StringUtil.nvl(box.getString("p_tutortype"),"T");
			...
			<td class="border_gray02" align="center">
				...
				<tr>
					<td class="txt_b">
						...
						튜터구분 
						<input type="radio" name="p_tutorgubun" value="1" style="border:none;" onclick="selectTutorGubun('1');" <%=(v_tutorgubun.equals("1"))?" checked":"" %>> 사내튜터
						<input type="radio" name="p_tutorgubun" value="2" style="border:none;" onclick="selectTutorGubun('2');" <%=(v_tutorgubun.equals("2"))?" checked":"" %>> 사외튜터
					</td>
				</tr>
				</table>
			</td>
			...
			<tr>
				...
				<td>
					<% if( boxTutorInfo.getString("POST2") == null || boxTutorInfo.getString("POST2").equals("")){ %>
						<input name="p_post1" type="text" size="5" dataType="integer" dispName="우편번호" isNull="Y" maxLength="5" value="<%=boxTutorInfo.getString("POST1") %>" readonly="readonly" />
						<input name="p_post2" type="hidden" dispName="우편번호" value="<%=boxTutorInfo.getString("POST2") %>"/>
					<% } else { %>
						<input name="p_post1" type="text" size="5" dataType="integer" dispName="우편번호" isNull="Y" maxLength="5" value="<%=boxTutorInfo.getString("POST1") %>" readonly="readonly" />
						-
						<input name="p_post2" type="text" size="5" dispName="우편번호" value="<%=boxTutorInfo.getString("POST2") %>" readonly="readonly" />
					<% } %>
				...
				<th>이메일</th>
				<td>
					<input name="p_email" type="text" size="40" dispName="이메일" isNull="Y" lenCheck="50" maxLength="50" value="<%=boxTutorInfo.getString("EMAIL") %>"/>
				</td>
			</tr>
			<tr>
<%
...
<%	}%>
			...
			<tr>
				<th colspan="2">자격증</th>
				<td colspan="3">
					<textarea name="p_license" cols="130" rows="5" dispName="자격증" isNull="Y" lenCheck="1000" maxLength="1000"><%=boxTutorInfo.getString("LICENSE") %></textarea><br/>
				</td>
			</tr>
			<tr>
				<th rowspan="2">튜터소개</th>
				<th class="th_left_bd">강의분야</th>
				<td colspan="3">
					<input type="text" name="p_professional" size="50"  dispName="강의분야" isNull="Y" lenCheck="20" maxLength="20" value="<%=boxTutorInfo.getString("PROFESSIONAL") %>"><br/>
				</td>
			</tr>
			<tr>
				<th class="th_left_bd">담당자의견</th>
				<td colspan="5">
					<input type="text" name="p_opinion" size="50"  dispName="담당자의견" isNull="Y" lenCheck="1000" maxLength="1000" value="<%=boxTutorInfo.getString("OPINION") %>"><br/>
				</td>
			</tr>
			<tr>
				<th rowspan="2">튜터권한</th>
				<th class="th_left_bd">튜터권한</th>
				<td colspan="3">
			...
			<tr>
				<th colspan="2">튜터이력서</th>
				<td colspan="3">
				<%	if ( !boxTutorInfo.getString("REALRESUME").equals("") ) { %>
					<span id="OldFileResume"><a href="#" onclick="fileDown('<%=boxTutorInfo.getString("REALRESUME") %>', '<%=boxTutorInfo.getString("RESUME") %>')"><%=boxTutorInfo.getString("REALRESUME") %></a>&nbsp;
			...
			<tr>
				...
				<%	if ( !boxTutorInfo.getString("REALCERTIFICATE").equals("") ) { %>
					<span id="OldFileCertificate"><a href="#" onclick="fileDown('<%=boxTutorInfo.getString("REALCERTIFICATE") %>', '<%=boxTutorInfo.getString("CERTIFICATE") %>')"><%=boxTutorInfo.getString("REALCERTIFICATE") %></a>&nbsp;
