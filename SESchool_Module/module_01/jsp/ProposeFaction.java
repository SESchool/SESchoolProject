...
public class ProposeFaction extends StrutsDispatchAction {
	...
	throws Exception {
		...
		String v_trainingarea = boxProposeApplyInfo.getString("TRAININGAREA");  
		...
		box.put("p_trainingarea", v_trainingarea);
	...
	throws Exception {
		...
		if(Integer.parseInt(v_installment_seq) < 2){	
	...
	throws Exception {
		...
		box.put("subjSeqInfo", subjSeqMgr.getSubjSeq(box).getFirstObject());
		box.put("subjSeqSubBL", subjSeqMgr.getSubjSeq(box).getFirstObject()); 
	...
	throws Exception {
		...
		} else if(proposeInfo.getString("CHKFINAL").equals("Y") || proposeInfo.getString("CHKFINAL").equals("승인")) { 
